...
namespace Galactica::Math
{
	...
	template <typename T>
	void Mat3<T>::InitializeClassWithSol(const std::shared_ptr<sol::state>& luaState)
	{
		luaState->new_usertype<Mat3>(
			"Mat3",

			"new", sol::constructors<
				Mat3(T, T, T, T, T, T, T, T, T), 
				Mat3(const Vec3<T>&, const Vec3<T>&, const Vec3<T>&),
				Mat3(T),
				Mat3(const Mat3&)>(),

			"GetValueAtIndex", &Mat3::GetValueAtIndex,
			"SetValueAtIndex", &Mat3::SetValueAtIndex,

			sol::meta_function::to_string, &Mat3::ToString,

			sol::meta_function::multiplication, sol::overload(
				sol::resolve<Mat3(const Mat3&)>(&Mat3::Multiply), 
				sol::resolve<Mat3(T)>(&Mat3::Multiply)),

			"Inverted",		&Mat3::Inverted,
			"Invert",		&Mat3::Invert,
			"Transposed",	&Mat3::Transposed,
			"Transpose",	&Mat3::Transpose,
			"Determinant",	&Mat3::Determinant,
			"GetColumn",	&Mat3::GetColumn,
			"SetColumn",	&Mat3::SetColumn,
			"GetRow",		&Mat3::GetRow,
			"SetRow",		&Mat3::SetRow
		);

		luaState->set_function("MakeRotationX", &Mat3::MakeRotationX);
		luaState->set_function("MakeRotationY", &Mat3::MakeRotationY);
		luaState->set_function("MakeRotationZ", &Mat3::MakeRotationZ);
		luaState->set_function("MakeRotation",	&Mat3::MakeRotation);

		luaState->set_function("MakeScale", sol::overload(
				sol::resolve<Mat3(T)>(&Mat3::MakeScale),
				sol::resolve<Mat3(T, T, T)>(&Mat3::MakeScale),
				sol::resolve<Mat3(const Vec3<T>&)>(&Mat3::MakeScale)));

		luaState->set_function("Identity",		&Mat3::Identity);
	}

	template <typename T>
	T Mat3<T>::GetValueAtIndex(int i, int j)
	{
		return data[i][j];
	}

	template <typename T>
	void Mat3<T>::SetValueAtIndex(int i, int j, T value)
	{
		data[i][j] = value;
	}

	template <typename T>
	Mat3<T> Mat3<T>::Multiply(const Mat3& matrix)
	{
		return (*this) * matrix;
	}

	template <typename T>
	Mat3<T> Mat3<T>::Multiply(T scalar)
	{
		return (*this) * scalar;
	}
}
